...
class PostgresStore implements Store {
    ...
    async increment(key: string): Promise<ClientRateLimitInfo> {
        ...
        let recordInsertQuery = 'INSERT INTO rate_limit.records(key, session_id) VALUES ($1, $2)'
        let numberOfRecordsQuery = 'SELECT count(id) AS count FROM rate_limit.records WHERE key = $1'
    ...
    async getSession(): Promise<Session> {
        let selectSessionQuery = 'SELECT id, expires_at FROM rate_limit.sessions LIMIT 1'
        try {
    ...
    async createNewSession(): Promise<Session> {
        ...
        let insertSessionQuery = 'INSERT INTO rate_limit.sessions(expires_at) SELECT $1 RETURNING id'

        let client = await this.pool.connect()
        try {
            await client.query(deleteSessionQuery)
            let result = await client.query(insertSessionQuery, [newSession.expires_at])
